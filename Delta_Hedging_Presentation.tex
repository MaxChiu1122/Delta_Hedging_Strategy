...
\definecolor{primary}{RGB}{0,76,153}      % deep blue
...
\author{Max Chiu}
\institute{Personal Quantitative Research Project}
...
\begin{frame}{Instruments \& Conventions}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \textbf{Transaction costs} (TAIFEX schedule)
      \begin{itemize}\small
        \item TX: exch.\ fee \nt{20} $+$ tax $0.00002{\times}200F$ ($\approx$\nt{108}) per contract $\times|\Delta h|$
        \item TXO: exch.\ fee \nt{10} $+$ tax $0.001{\times}$prem ($\approx$\nt{13}) at inception
        \item Broker commission excluded; slippage 0
      \end{itemize}
    \column{0.5\textwidth}
  ...
  \begin{columns}[T]
    \column{0.46\textwidth}
      ...
      \begin{tabular}{lr}
        ...
        Transaction costs & \loss{$-74$} \\
        \midrule
        \textbf{Net P\&L} & \loss{$\mathbf{-34{,}380}$} \\
        \bottomrule
      \end{tabular}
      \vspace{0.5em}
...
\begin{frame}{Model 2 --- Sticky-Strike vs Sticky-Delta}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \end{itemize}
      \begin{block}{Result}
        \small Sticky-Delta net \loss{$-$\nt{39{,}191}} --- \emph{worse}. Its larger prior short lost more on the Apr 10 rally (whipsaw).
      \end{block}
    \column{0.5\textwidth}
      \centering
  ...
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \begin{itemize}\small
        ...
        \item $\beta$ from rolling 252-day (1-year) regression, initialised pre-$t_0$, updated daily (no lookahead).
      \end{itemize}
  ...
  \begin{columns}[T]
    \column{0.46\textwidth}
      ...
      \begin{alertblock}{Result --- sophistication backfired}
        \small Net \loss{$-$\nt{38{,}330}} \;(\loss{$-$\nt{3{,}950}} vs.\ baseline). Degenerate $v_0\!\approx\!0.015$ $\Rightarrow$ too-small delta $\Rightarrow$ under-hedged into the fall. \textbf{Estimation/calibration risk} during crisis.
      \end{alertblock}
    \column{0.54\textwidth}
      \centering
...
\begin{frame}{Model 5 --- A Genuine, Lower-Turnover Hedge}
  \begin{columns}[T]
    \column{0.46\textwidth}
      ...
      \begin{itemize}\small
        \item Carries a real hedge: mean $|h|\approx0.073$ vs.\ BS $0.097$.
        \item \textbf{Front-loads} the hedge in the calm period; ramps smoothly.
        \item \textbf{Under-hedges the crash bottom} --- holds 33/46/59\% of BS delta on Apr 7--9.
        \item \textbf{Turnover 50\% below BS} (0.22 vs.\ 0.43) $\Rightarrow$ lowest cost, less whipsaw.
      \end{itemize}
...
\begin{frame}{Cross-Model Scorecard}
  ...
  \begin{tabular}{clrr}
    ...
    \midrule
    \rowcolor{lightblue}
    1 & Model 5 --- Deep Hedging & $-31{,}434$ & \gain{$+2{,}946$} \\
    2 & Model 3 --- MV Delta & $-32{,}244$ & \gain{$+2{,}136$} \\
    3 & Model 1 --- Black-76 (baseline) & $-34{,}380$ & --- \\
    4 & Model 4 --- Heston SV & $-38{,}330$ & \loss{$-3{,}950$} \\
    5 & Model 2b --- Sticky-Delta & $-39{,}191$ & \loss{$-4{,}811$} \\
    \bottomrule
  \end{tabular}
  ...
  \begin{columns}[T]
    \column{0.5\textwidth}
      \centering\includegraphics[width=\linewidth,keepaspectratio,height=0.40\textheight]{fig_m5_comparison.png}
    \column{0.5\textwidth}
      \small
      \begin{itemize}
        \item Every model lost money --- a 9\% OTM put through a jump is structurally hard to hedge.
        \item The spread from best to worst is \textbf{\nt{7{,}757}} --- model choice clearly matters.
      \end{itemize}
  \end{columns}
\end{frame}
