  ...
  \begin{columns}[T]
    ...
    \column{0.46\textwidth}
      \begin{alertblock}{Choosing $\alpha$ --- the key design call}
        \small
        $\alpha{=}0.50$ (original notebook's training value) averages the worst \emph{half} $\Rightarrow$ degenerates to "do not hedge."\\[0.3em]
        I use \textbf{$\alpha{=}0.95$ (95\% Expected Shortfall)} --- a principled, conventional tail measure, applied to both training and evaluation.
      \end{alertblock}
  \end{columns}
\end{frame}
...
\begin{frame}{Key Findings}
  ...
  \begin{block}{3. The objective \emph{is} the strategy}
    Deep Hedging only works once the risk measure is right: $\alpha{=}0.50 \to$ "never hedge"; $\alpha{=}0.95 \to$ an efficient, whipsaw-aware hedge that won for the \textbf{right} reason (turnover control).
  \end{block}
\end{frame}
